...
async function downloadPNG(logo: LogoConfig, previewRef: React.RefObject<HTMLDivElement>, size: number, label: string) {
  ...
  a.download = `${logo.name}-${label}.png`;
  a.href = dataUrl;
  a.click();
}
...
async function downloadSVG(logo: LogoConfig) {
  ...
  <rect width="320" height="120" rx="12" fill="${palette.bg}"/>
  ${inner}
</svg>`;
...
async function downloadFavicon(logo: LogoConfig, previewRef: React.RefObject<HTMLDivElement>) {
  ...
  const a = document.createElement("a");
  a.download = `${logo.name}-favicon.png`;
  a.href = canvas.toDataURL("image/png");
  a.click();
}

const FILES = [
  { label: "Logo PNG", desc: "2000×800px — for web & print", icon: "🖼", action: "png-large" },
  { label: "Logo PNG Small", desc: "500×200px — for emails & docs", icon: "📄", action: "png-small" },
  { label: "Logo SVG", desc: "Vector — scales to any size", icon: "✦", action: "svg" },
  { label: "Favicon", desc: "32×32px — for browser tab", icon: "⭐", action: "favicon" },
];

export default function DownloadPackage({ logo, previewRef, paid, onUnlock }: DownloadPackageProps) {
  async function handleDownload(action: string) {
    ...
    if (action === "png-small") await downloadPNG(logo, previewRef, 500, "500px");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {!paid && (
        <div style={{
          ...
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Unlock all files — $5</div>
          <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 12 }}>PNG, SVG, Favicon + future updates</div>
      ...
      {FILES.map(f => (
        <div key={f.action} onClick={() => handleDownload(f.action)} style={{
          ...
          onMouseEnter={e => { if(paid) e.currentTarget.style.borderColor = "#4f46e5"; }}
